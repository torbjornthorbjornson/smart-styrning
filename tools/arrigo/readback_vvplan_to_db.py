...
"""readback_vvplan_to_db.py  (WORKER / LIBRARY)

ANSVAR:
- Läser VV_PLAN(0..95) från Arrigo via redan inloggad token (injectad från orchestrator)
...
def read_vv_plan_96(gql_fn, token: str, pvl_b64: str, prefix: str) -> list[int]:
...
def main():
    ...
    vv = read_vv_plan_96(orch.gql, token, pvl_b64, prefix=prefix)
    upsert_plan(site_code, plan_type, day_local, vv)
