...
"""
push_from_db.py
- Hämtar elpriser (24 eller 96 perioder beroende på ARRIGO_PERIODS)
...
PERIODS = int(os.getenv("ARRIGO_PERIODS", "24"))
...
def build_rank_and_masks(rows):
    ...
    mid = PERIODS // 2
    median = (sorted_prices[mid-1] + sorted_prices[mid])/2.0 if PERIODS % 2 == 0 else sorted_prices[mid]
    ...
    ec_masks = pack_masks(cheap_slots, PERIODS)
    ex_masks = pack_masks(exp_slots, PERIODS)
...
def push_to_arrigo(rank, ec_masks, ex_masks, day_local, oat_yday, oat_tmr, slot_price):
    ...
    for i,v in enumerate(vars_list):
        ...
        # Matcha PRICE_RANK(n)
        m_rank=re.search(r"PRICE_RANK\((\d+)\)$", ta, re.IGNORECASE)
        if m_rank:
            idx_rank[int(m_rank.group(1))]=i
            continue

        # Matcha PRICE_VAL(n)
        m_val=re.search(r"PRICE_VAL\((\d+)\)$", ta, re.IGNORECASE)
        if m_val:
            idx_vals[int(m_val.group(1))]=i
            continue
        ...
        # Masker 32-bit
        m_ec=re.search(r"EC_MASK32_(\d+)$", ta, re.IGNORECASE)
        if m_ec: idx_ec32[int(m_ec.group(1))]=i; continue

        m_ex=re.search(r"EX_MASK32_(\d+)$", ta, re.IGNORECASE)
        if m_ex: idx_ex32[int(m_ex.group(1))]=i; continue

    log(f"🧭 PRICE_RANK map: {sorted(idx_rank.items())}")
    log(f"🧭 Price_Val  map: {sorted(idx_vals.items())}")

    if idx_price_ok is not None:
        gql(graphql_url,token,"mutation($v:[VariableKeyValue!]!){writeData(variables:$v)}",
            {"v":[{"key":f"{pvl_b64}:{idx_price_ok}","value":"0"}]},verify)
        log("🔒 PRICE_OK=0")
    ...
    log(f"🧩 EC_MASK32={ec_masks}, EX_MASK32={ex_masks}")
    ...
    if idx_price_ok is not None:
        gql(graphql_url,token,"mutation($v:[VariableKeyValue!]!){writeData(variables:$v)}",
            {"v":[{"key":f"{pvl_b64}:{idx_price_ok}","value":"1"}]},verify)
        log("🔓 PRICE_OK=1")
