...
def create_backup_tag():
    try:
        comment = request.form.get("comment", "").strip().replace(":", "-").replace("
", " ")
        now = datetime.now().strftime("%Y%m%d_%H%M")
        tag_name = f"backup_{now}"
        branch_name = f"snapshot_{now}"
        message = f"🧷 Backup via webben {now}" + (f": {comment}" if comment else "")

        # Kontrollera att vi inte är i detached HEAD-läge
        current_branch = subprocess.check_output(["/usr/bin/git", "rev-parse", "--abbrev-ref", "HEAD"]).decode().strip()
        if current_branch == "HEAD":
            return "❌ Du är i detached HEAD-läge – växla tillbaka till main innan backup."

        # Lägg till ändringar och skapa commit
        subprocess.run(["/usr/bin/git", "add", "."], check=True)
        subprocess.run(["/usr/bin/git", "commit", "-m", message], check=True)

        # Skapa tagg och branch
        subprocess.run(["/usr/bin/git", "tag", "-a", tag_name, "-m", message], check=True)
        subprocess.run(["/usr/bin/git", "branch", branch_name], check=True)

        # Pusha till GitHub
        subprocess.run(["/usr/bin/git", "push", "origin", "main"], check=True)
        subprocess.run(["/usr/bin/git", "push", "origin", tag_name], check=True)
        subprocess.run(["/usr/bin/git", "push", "origin", branch_name], check=True)

        return render_template("restore_result.html", tag=tag_name, backup_tag=branch_name)

    except subprocess.CalledProcessError as e:
        return f"❌ Git-fel vid backup: {e}"
    except Exception as e:
        return f"❌ Allmänt fel vid backup: {e}"
