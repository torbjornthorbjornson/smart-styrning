...
from smartweb_backend.db.prices_repo import insert_ignore_electricity_prices
...
def parse_and_save(data, target_date_local):
    ...
    for row in data:
        ...
        entries.append((ts_utc_naive, float(price)))

    inserted = insert_ignore_electricity_prices(entries)
    saved += inserted

    for ts_utc_naive, price in entries:
        logging.info(f"💾 (försökt) {ts_utc_naive}Z => {price} kr/kWh")
